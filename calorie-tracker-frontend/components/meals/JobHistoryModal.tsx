      ...
      >
        <header className="flex items-start justify-between border-b border-zinc-100 px-5 py-5 sm:px-6">
          ...
          <button
            ...
            onClick={onClose}
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-900 disabled:opacity-40"
          >
        ...
        <div className="min-h-64 flex-1 overflow-y-auto p-5 sm:p-6">
          ...
          {isLoading ? (
            ...
            <div className="flex min-h-56 flex-col items-center justify-center text-center">
              ...
              <p className="mt-4 text-sm font-semibold text-zinc-900">No imports yet</p>
              <p className="mt-1 text-sm text-zinc-600">Uploaded PDF diaries will appear here.</p>
            </div>
          ) : (
    ...
    <article className="rounded-xl border border-zinc-200 bg-white p-4 shadow-[0_1px_2px_rgb(24_24_27/0.03)]">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {job.failureReason && (
            <p className="mt-2 break-words rounded-lg bg-red-50 px-3 py-2 text-xs leading-5 text-red-700">
              {job.failureReason}
            </p>
          )}
        </div>
        {job.status === "FAILED" && (
          <button
            ...
            onClick={onRetry}
            className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-3 text-xs font-semibold text-zinc-700 shadow-sm transition hover:bg-zinc-50 disabled:cursor-wait disabled:opacity-50"
          >
            {isRetrying ? (
...
function StatusBadge({ status }: { status: ImportJobStatus }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[0.65rem] font-bold", styles[status])}>
