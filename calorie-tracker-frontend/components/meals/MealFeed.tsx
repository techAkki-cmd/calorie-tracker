        ...
        <div className="flex flex-1 items-center justify-center p-6 sm:p-10">
          <div className="max-w-sm text-center">
            ...
            <button
              ...
              className="mt-4 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm font-semibold text-zinc-700 shadow-sm transition hover:bg-zinc-50"
            >
              Try again
            </button>
          </div>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex flex-1 items-center justify-center p-6 sm:p-10">
          <div className="flex max-w-sm flex-col items-center text-center">
            <span className="flex h-14 w-14 items-center justify-center rounded-2xl border border-zinc-200 bg-zinc-50 text-zinc-500 shadow-sm">
...
function MealFilters({
  ...
  return (
    <div className="space-y-3 border-b border-zinc-100 bg-zinc-50/50 px-4 py-3 sm:px-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <input
            ...
            className={cn(
              "h-10 rounded-lg border bg-white px-2 text-sm font-medium text-zinc-700 shadow-sm outline-none transition focus:border-transparent focus:ring-2 focus:ring-teal-600",
          ...
          <button
            ...
            className={cn(
              "ml-1 h-10 rounded-lg px-2.5 text-sm font-semibold transition",
              isLastSevenDays
          ...
          <button
            ...
            className={cn(
              "h-10 rounded-lg px-2.5 text-sm font-semibold transition",
              isTodayOnly
                ? "bg-teal-600 text-white shadow-sm"
                : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900",
            )}
      ...
      {rangeError && (
        <p id="meal-feed-range-error" className="text-sm font-medium text-red-600" role="alert">
          {rangeError}
        </p>
      )}
      <div
      ...
      >
        {MEAL_TYPE_FILTERS.map((filter) => (
          <button
            ...
            className={cn(
              "h-10 min-w-0 rounded-md px-1.5 text-sm font-semibold transition",
...
function PdfSyncIndicator({ active }: { active: boolean }) {
  ...
  return (
    <div
      className="flex items-center gap-2 border-b border-teal-100 bg-teal-50/60 px-5 py-2 text-sm font-medium text-teal-700"
      role="status"
      aria-live="polite"
    >
...
function PdfSyncError({ message }: { message?: string }) {
  ...
  return (
    <div
      className="flex items-center gap-2 border-b border-red-100 bg-red-50/70 px-5 py-2 text-sm font-medium text-red-700"
      role="alert"
    >
...
function MealCard({
  ...
  return (
    <article className="group relative flex min-h-[7.25rem] gap-3 rounded-xl border border-zinc-200 bg-white p-4 shadow-[0_1px_2px_rgb(24_24_27/0.03)] transition hover:border-zinc-300 hover:shadow-sm">
      <span className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-100 text-zinc-700">
        <UtensilsCrossed className="h-5 w-5" aria-hidden />
      </span>
      <div className="min-w-0 flex-1 pr-7">
        <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-1">
          <div className="min-w-0">
            <h3 className="truncate text-lg font-semibold text-zinc-900">{meal.name}</h3>
            <p className="mt-0.5 text-sm text-zinc-600">
              {toTitleCase(meal.mealType)} · {meal.quantity}
            </p>
          </div>
          <p className="inline-flex shrink-0 items-center gap-1.5 text-sm tabular-nums text-zinc-500">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <MacroPill label="Protein" value={meal.protein} className="bg-slate-100 text-slate-800" />
          <MacroPill label="Carbs" value={meal.carbs} className="bg-slate-50 text-slate-700 ring-1 ring-inset ring-slate-200" />
          <MacroPill label="Fat" value={meal.fat} className="bg-teal-50 text-teal-800" />
          <span className="rounded-full bg-zinc-900 px-3 py-1.5 text-sm font-bold tabular-nums text-white">
            {meal.calories} kcal
          </span>
        </div>
        {meal.micronutrientSummary && (
          <span className="mt-2 inline-flex max-w-full items-center gap-1.5 rounded-full border border-teal-100 bg-teal-50/70 px-2.5 py-1 text-sm text-zinc-700">
      ...
      <button
        ...
        disabled={isDeleting}
        onClick={onDelete}
        className="absolute right-3 top-3 inline-flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 opacity-100 transition hover:bg-red-50 hover:text-red-600 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:cursor-wait disabled:opacity-60 sm:opacity-0 sm:group-hover:opacity-100"
      >
        {isDeleting ? (
...
function MacroPill({
  ...
  return (
    <span className={`rounded-full px-3 py-1.5 text-sm font-semibold tabular-nums ${className}`}>
...
function PaginationButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="inline-flex h-10 items-center gap-1 rounded-lg border border-zinc-200 bg-white px-2.5 text-sm font-medium text-zinc-600 shadow-sm transition hover:bg-zinc-50 hover:text-zinc-900 disabled:cursor-not-allowed disabled:opacity-40"
    >
      {!iconAfter && icon}
      {label}
      {iconAfter && icon}
